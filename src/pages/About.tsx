...
export const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSending(true);
    const mailSubject = encodeURIComponent(`[EuroVize] ${subject}`);
    ...
    setTimeout(() => {
      setSending(false);
      showToast('Varsayılan e-posta uygulamanız açıldı.', 'success');
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-2xl border border-gray-100 p-8">
                ...
                <p className="text-sm text-gray-500 mb-6">Formu doldurduğunuzda varsayılan e-posta uygulamanız açılır ve mesajınız hazır şekilde oluşturulur.</p>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button type="submit" disabled={sending} className="w-full px-6 py-3.5 bg-[#0f2b5b] text-white rounded-xl font-bold hover:bg-[#1a3f7a] transition-colors text-lg disabled:opacity-60">
                    {sending ? 'Hazırlanıyor...' : 'E-posta ile Gönder'}
                  </button>
                </form>
              </div>
            </div>
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError('');
    const ok = await login(sanitizeEmail(email), password);
    setLoading(false);
    if (ok) {
      navigate(isAdminEmail(sanitizeEmail(email)) ? '/admin' : '/panel');
    } else {
...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const ok = await register(sanitizeText(name, 120), sanitizeEmail(email), password, sanitizePhone(phone));
    setLoading(false);
    if (ok) {
      navigate(isAdminEmail(sanitizeEmail(email)) ? '/admin' : '/panel');
    } else {
